...
#  $\color{lightblue}\sf{LF}$   <br>   Online Multi-Robot Path Planning Meets Optimal Trajectory Control   
---

$\color{lightblue}\sf{LF}$ is a multi-robot control paradigm to solve point-to-point navigation tasks for a _team_ of holonomic robots with access to the full environment information. It designs a heirarchical structure to multi-robot planning and control: it invokes two processes asynchronously at high frequency:
...
At the moment, $\color{lightblue}\sf{LF}$ can trigger its planner at upto 20Hz on a laptop CPU &mdash; this can be   
